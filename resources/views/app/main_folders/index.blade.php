    ...
    <div class="table-responsive">
        <table class="table card-table table-vcenter text-nowrap datatable">
            <thead>
                <tr>
                    <th class="text-left">
                        @lang('crud.main_folders.inputs.name')
                    </th>
                    <th class="text-center">@lang('crud.common.actions')</th>
                </tr>
            </thead>
            <tbody>
                @forelse($mainFolders as $mainFolder)
                <tr>
                    <td>{{ $mainFolder->name ?? '-' }}</td>
                    <td class="text-center" style="width: 134px;">
                        <div
                            ...
                            class="btn-group"
                        >
                            @can('update', $mainFolder)
                            <a
                                href="{{ route('main-folders.edit', $mainFolder) }}"
                                class="btn btn-icon btn-outline-warinig ms-1"
                            >
                                <i class="ti ti-edit"></i>
                            </a>
                            @endcan @can('view', $mainFolder)
                            <a
                                href="{{ route('main-folders.show', $mainFolder) }}"
                                class="btn btn-icon btn-outline-info ms-1"
                            >
                                <i class="ti ti-eye"></i>
                            </a>
                            @endcan @can('delete', $mainFolder)
                            <form
                                action="{{ route('main-folders.destroy', $mainFolder) }}"
                                method="POST"
